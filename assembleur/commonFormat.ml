...
let int32_to_word x = 
  ...
  let x0 = x / shift.(0) in
  let x1 = (x % shift.(0)) / shift.(1) in
  let x2 = (x % shift.(1)) / shift.(2) in
  let x3 = x % shift.(2) in
...
let int32_to_half x = 
  let y = Int32.abs x in
  let sgn = if x >= 0l then 1l lsl 7 else 0l in
  let _ = assert ( y < 1l lsl 15)  in
  let shift = 1l lsl 8 in
  let x0 = y / shift + sgn in
  let x1 = y % shift in
    List.map chr [ x1 ; x0 ]
