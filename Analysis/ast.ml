...
struct
  ...
  let rec int_power x = function 
	...
	| 0 -> let y = int_power x (n/2) in y*y 
	| 1 -> let y = int_power x (n/2) in y*y*x
	| _ -> failwith "cas impossible : n mod 2 = 0 ou 1"
...
module type AstModuleType =
sig

  include IntModuleType
 ...
 type wire =
  ...
  (* Définition d'un type de périphérique 
     ...
     id -> nom du device
     int -> nombre d'entrée du device nécéssaire pour l'analyse sémantique
  *)
...
module IntegerAst = (Make(Integer) : AstModuleType with type integer = Integer.integer)
