...
\end{itemize}
\text{}\\
Les blocs classiques se construisent par composition. Ils ont en général la forme
suivante : 
\begin{verbatim}
...
coder des blocs récursivement - et dans les fils - pour pouvoir préciser leur taille.
\text{}\\
...
couper une boucle combinatoire, on perd simplement un cycle à attendre.
...
\item Un premier parcours de l'AST détermine la liste des portes
  ...
  (cette transformation permet notamment d'obtenir un accès aléatoire à chaque
  élément).
...
\begin{format}[codage du I-format]
  ...
  \item 0..5 : les 6 bits de l'opcode
  \item 6..10 : adresse \texttt{rs} (opérande 1)
  \item 11..15 : adresse \texttt{rt} (résultat)
...
\end{itemize}
\text{}\\
Les conventions de nommage employées pour les registres sont celles appelées
...
\item[$\rhd$ Resources :] contient des fichiers nécessaires pour le fonctionnement
  d'Obsidian.
